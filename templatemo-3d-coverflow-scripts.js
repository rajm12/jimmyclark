...
// JavaScript Document
        ...
        const imageData = [
            {
                title: "Silk Vanilla",
                description: "The smoothest, silkest vanilla experience ever"
            },
            {
                title: "Deep Choco",
                description: "Dive into the deep, rich chocolate intensity"
            },
            {
                title: "Pista Prime",
                description: "Premium roasted pistachio in every bite"
            },
            {
                title: "Mango Rush",
                description: "A bold rush of tropical mango energy"
            },
            {
                title: "Berry Blast",
                description: "A vibrant explosion of sweet strawberry freshness"
            }
        ];
...
function updateCountdowns() {
    const cards = document.querySelectorAll('.game-card-compact');
    const now = new Date();
    ...
    cards.forEach(card => {
        ...
        const display = card.querySelector('.days-count');
        const countdownBox = card.querySelector('.countdown-mini');
        if (!display || !countdownBox) return;

        if (daysLeft > 0) {
            display.innerText = daysLeft;
        } else if (daysLeft === 0) {
            countdownBox.innerHTML = "TODAY";
            countdownBox.style.color = "#27ae60";
...
function loadGamesSection() {
    fetch('games.html')
      ...
      .then(data => {
        document.getElementById('games-placeholder').innerHTML = data;
        
        // הפעלה ראשונה מיד כשנטען
        updateCountdowns(); 

        // הפעלה מחזורית בכל שעה - כאן זה המקום הנכון!
        setInterval(updateCountdowns, 1000 * 60 * 60); 
      })
      .catch(error => console.error('Error loading the section:', error));
}

// הפעלה ראשונית של התהליך
document.addEventListener('DOMContentLoaded', loadGamesSection);
